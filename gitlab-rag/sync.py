...
def _unwrap_obj(payload):
    ...
    if isinstance(payload, dict):
        # If the top-level object already has an 'id', it IS the object — don't unwrap.
        # (OpenWebUI uses 'data' as a status/metadata field, not always a wrapper.)
        if 'id' in payload:
            return payload
...
def get_or_create_knowledge():
    r = requests.get(f"{OPENWEBUI_URL}/api/v1/knowledge/", headers=ow_headers(), timeout=30)
    r.raise_for_status()
...
def reset_knowledge(kb_id):
    # Reset can be slow on a populated KB — give it room
    r = requests.post(f"{OPENWEBUI_URL}/api/v1/knowledge/{kb_id}/reset", headers=ow_headers(), timeout=120)
    r.raise_for_status()
...
def upload_and_index(kb_id, project_id, path, content):
    ...
    # Upload step. The embedding happens server-side here, so this can be slow
    # on large files or a busy Ollama. Generous timeout.
    try:
        r = requests.post(
    ...
    try:
        ...
        if not file_id:
            print(f"        upload returned no id: {r.text[:200]}")
            return False
    except ValueError:
        print(f"        upload returned non-JSON: {r.text[:200]}")
        return False

    # Attach step. Also embeds server-side on some OpenWebUI versions.
    try:
        r = requests.post(
        ...
        return False
    if not r.ok:
        print(f"        attach HTTP {r.status_code}: {r.text[:200]}")
        return False
    return True
